...
# This script must be run from the cloned repo via: npm run setup
if [ ! -f "./init-scanner.sh" ]; then
  echo "Error: init-scanner.sh not found in current directory"
  echo ""
  echo "This setup script should be run via npm run setup:"
  echo "  git clone https://github.com/Millstone123/m100-dev-setup.git"
  echo "  cd m100-dev-setup"
  echo "  npm run setup"
  echo ""
  exit 1
fi
